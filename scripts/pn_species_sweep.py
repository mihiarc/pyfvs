#!/usr/bin/env python3
"""Exhaustive PN species parity sweep: pyfvs vs native FVSpn for all 39 species.

Reporting-only discovery tool. Mirrors ne_species_sweep.py. Writes to
``test_output/pn_species_sweep.md``.

Default scenario per species:
  - 500 TPA, 30 years (3 PN 10-year cycles), bare_ground=True
  - site_index: 100 (typical productive PN site)
...
Usage:
    uv run python scripts/pn_species_sweep.py
"""
...
# Fortran pn/blkdat.f:134-140 DATA JSP — 40 positions, 1 blank at
# position 39 (between WI and OT). 39 real species.
PN_SPECIES = [
    'SF', 'WF', 'GF', 'AF', 'RF', 'SS', 'NF',      # 1-7
...
assert len(PN_SPECIES) == 39
assert sum(1 for s in PN_SPECIES if s is not None) == 38
...
def run_one_species(species: str, site_index: int) -> dict:
    ...
    try:
        py = run_pyfvs_multi_seed(
            variant="PN", species=species, site_index=site_index,
...
def render_report(results, elapsed):
    ...
    return f"""# PN Species Parity Sweep

**Generated:** {now}
**Elapsed:** {elapsed:.1f}s
**Scenario:** {DEFAULT_TPA} TPA, {DEFAULT_YEARS}yr (3x PN 10-year cycles), bare_ground=True, {N_SEEDS} pyfvs seeds (base={BASE_SEED}) vs 1 native run
...
def main():
    ...
    if not fvs_library_available("PN"):
        print("ERROR: FVSpn native library not found.", file=sys.stderr)
        return 0

    species_to_run = [s for s in PN_SPECIES if s is not None]
    print(f"PN sweep: {len(species_to_run)} species, {N_SEEDS} pyfvs seeds, "
    ...
    out_path = REPO_ROOT / "test_output" / "pn_species_sweep.md"
